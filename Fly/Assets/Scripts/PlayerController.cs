...
public class PlayerController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        MaxBoosterSpeed = PlayerData.MaxBoosterSpeed;
    ...
    public void _updateEnergy(float amt)
    {
        _curEnergy += amt;
        if (_curEnergy <= 0f)
        {
        ...
        else if (_curEnergy >= MaxEnergy)
        {
            _curEnergy = MaxEnergy;
        }
        EnergyBar.fillAmount = _curEnergy / MaxEnergy;
    }
    ...
    private abstract class PlayerOnGroundState : PlayerState
    {
        public override void Update()
        {
            ...
            if (!_Jump && _isOnGround() && !Context.GetComponent<CharacterAbility>().EnergyCosting)
            {
            ...
            else if (_Jump)
            {
                ...
                if (Context._boosterForce >= Context.MaxBoosterSpeed ||
